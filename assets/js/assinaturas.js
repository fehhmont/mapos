    ...
    if(salvarAss) {
        salvarAss.addEventListener('click', function() {
            if (assClientePad.isEmpty() || assTecnicoPad.isEmpty()) {
                ...
                // Inserção do Token CSRF para evitar Erro 403
                if(window.csrfName) {
                    form_data.append(window.csrfName, window.csrfHash);
                }
    ...
    function post_form(form_data, assClienteImg64 = '', assTecnicoImg64 = '')
    {
        $.ajax({
            ...
            success: function(response){
                // Atualiza o Hash CSRF para a próxima requisição se o servidor retornar um novo
                if(response.token) {
                    window.csrfHash = response.token;
                }

                if(response.success == true) {
                    if(response.assClienteImg) {
                        assCliente && assCliente.remove();
            ...
            error: function (request, status, error) {
                 Swal.fire({
                    icon: "error",
                    title: "Erro 403/500",
                    text: "Erro de permissão ou segurança (CSRF). Tente atualizar a página."
                });
            }
        });
    }
